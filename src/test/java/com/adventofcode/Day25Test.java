...
public class Day25Test {
    ...
    private static long findLoop(long pk) {
        ...
        LOGGER.debug("findLoop({}) = {}", pk, loopNo);
        return loopNo;
    }

    private static long findEncryptionKey(long pk, long loopNo) {
        long value = 1;
        for (long i = 0; i < loopNo; i++) {
            value = loop(value, pk);
        }

        LOGGER.debug("getEncryptionKey({}, {}) = {}", pk, loopNo, value);
        return value;
    }

    private static long loop(long value, long seed) {
        value *= seed;
        value %= 20201227;
        return value;
    }

    private static long comboBreaker(long cardPublicKey, long doorPublicKey) {
        long cardLoop = findLoop(cardPublicKey);
        long doorLoop = findLoop(doorPublicKey);
        long cardEncryptionKey = findEncryptionKey(doorPublicKey, cardLoop);
        long doorEncryptionKey = findEncryptionKey(cardPublicKey, doorLoop);

        if (cardEncryptionKey != doorEncryptionKey)
            throw new IllegalStateException("Different EncryptionKey !");

        LOGGER.info("findEncryptionKey({}, {}) = {}", cardPublicKey, doorPublicKey, cardEncryptionKey);
        return cardEncryptionKey;
    }

    @Test
    void testComboBreaker() {
        long cardEncryptionKey = comboBreaker(5764801, 17807724);
     ...
     * <p>
     * "It's an emergency!" the Elf calling you explains. "The soft serve machine
     ...
     */
    @Test
    void inputComboBreaker() {
        Scanner scanner = new Scanner(Day25Test.class.getResourceAsStream("/day/25/input"));
        long cardPublicKey = Long.parseLong(scanner.nextLine());
        long doorPublicKey = Long.parseLong(scanner.nextLine());

        long encryptionKey = comboBreaker(cardPublicKey, doorPublicKey);
